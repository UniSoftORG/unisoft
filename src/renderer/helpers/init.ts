...
async function transformObject(componentData: IComponentType, shouldTransformByKey: string): Promise<IComponentType> {
  return processChildrenMapping({ ...componentData }, shouldTransformByKey, componentData, []);
}
...
async function prepareRequests(componentData: IComponentType){
  ...
  try {
    ...
    return Object.keys(requestedData ?? {}).reduce((acc: IComponentType['variables'], key) => {
...
function processChildrenMapping(checkObj: IComponentType, shouldTransformByKey: string, rootObj: IComponentType, parentHierarchy: IComponentType[]): IComponentType {
  ...
  for (const child of checkObj.children) {
    if (child.hasOwnProperty(shouldTransformByKey) && child[shouldTransformByKey].startsWith(rootObj.name)) {
      const parentValuePath = child[shouldTransformByKey].replace(`${rootObj.name}.`, "");
      const parentValue = getValue(rootObj, parentValuePath);
      if (Array.isArray(parentValue)) {
        newChildren.push(...parentValue.map((item, index) => createMappedChild(child, item, index, rootObj, updatedHierarchy)));
      }
    } else {
      newChildren.push(processChildrenMapping({ ...child }, shouldTransformByKey, rootObj, updatedHierarchy));
    }
  }

  return processReceiveAttributes({ ...checkObj, children: newChildren }, updatedHierarchy);
}

function createMappedChild(templateChild: IComponentType, data: any, index: number, rootObj: IComponentType, parentHierarchy: IComponentType[]): IComponentType {
  let newChild: IComponentType = { ...templateChild, passAttributes: { ...data, index }, uuid: v4() };
  if (templateChild.children) {
    newChild.children = templateChild.children.map((child: IComponentType) => processChildrenMapping({ ...child }, '', rootObj, [...parentHierarchy, newChild]));
  }
...
function processReceiveAttributes(obj: IComponentType, parentHierarchy: IComponentType[]): IComponentType {
  ...
  for (const key in obj.receiveAttributes) {
    ...
    for (let i = parentHierarchy.length - 1; i >= 0; i--) {
      ...
      if (!valuePath.includes(`.states.`) && valuePath.startsWith(`${parent.name}.`)) {
