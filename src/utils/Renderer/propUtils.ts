...
export function replaceWithStateValues(mainObject: AnyObject, passAttributes: AnyObject): AnyObject {
    ...
    for (const key in clonedPassAttributes) {
        if (typeof clonedPassAttributes[key] === 'object') {
            ...
            clonedPassAttributes[key] = replaceWithStateValues(mainObject, clonedPassAttributes[key]);
        } else if (typeof clonedPassAttributes[key] === 'string' &&
...
export function replaceWithValuesFromMainObject(mainObject: AnyObject, passAttributes: AnyObject, collectState: boolean = false): AnyObject {
    for (const key in passAttributes) {
        if (typeof passAttributes[key] === 'object') {
            replaceWithValuesFromMainObject(mainObject, passAttributes[key], collectState);
        } else if (typeof passAttributes[key] === 'string' &&
